        ...
        public DadosAlunos findStudentByName(string name)
        {
            ...
            try
            {
                try
                {
                    ...
                    sqlConnection.Open();
                    SqlCommand sqlCommand = new SqlCommand("STPECM_Consultas;5", sqlConnection)
                    {
                        CommandType = CommandType.StoredProcedure
                    };
                    sqlCommand.Parameters.Add("@Nome", SqlDbType.VarChar).Value = name;
                    sqlDataReader = sqlCommand.ExecuteReader();
                    dadosAluno = new DadosAlunos()
                    {
                        RetornoStudent = new List<Student>()
                    };
                    while (sqlDataReader.Read())
                    {
                        List<Student> retornoStudent = dadosAluno.RetornoStudent;
                        Student student = new Student()
                        {
                            CPFALUNO = Convert.ToString(sqlDataReader["CPFAluno"]),
                            RA = Convert.ToString(sqlDataReader["RA"]),
                            NOMEALUNO = Convert.ToString(sqlDataReader["NomeAluno"]),
                            CODCURSO = Convert.ToString(sqlDataReader["CodCurso"]),
                            NOMECURSO = Convert.ToString(sqlDataReader["NomeCurso"]),
                            CODCENTRO = Convert.ToString(sqlDataReader["CodCentro"]),
                            SIGLACENTRO = Convert.ToString(sqlDataReader["SiglaCentro"]),
                            NOMECENTRO = Convert.ToString(sqlDataReader["NomeCentro"])
                        };
                        retornoStudent.Add(student);
                    }
                }
                catch (Exception exception1)
                {
                    ...
                    createLogFile.ErrorLog(string.Concat("Metodo FindStudentByName", exception.Message));
                    dadosAluno = null;
                }
            }
            finally
        ...
        public DadosAlunos findStudentByRa(string ra, string usuario)
        {
            ...
            try
            {
                try
                {
                    sqlConnection = new SqlConnection(this.strConexaoBaseStudent);
                    sqlConnection.Open();
                    SqlCommand sqlCommand = new SqlCommand("dbo.STPECM_Consultas", sqlConnection)
                    {
                        CommandType = CommandType.StoredProcedure
                    };
                    sqlCommand.Parameters.Add(new SqlParameter("@ra", ra));
                    sqlDataReader = sqlCommand.ExecuteReader();
                    dadosAluno = new DadosAlunos()
                    {
                        RetornoStudent = new List<Student>()
                ...
                catch (Exception exception1)
                {
                    Exception exception = exception1;
                    CreateLogFiles createLogFile = new CreateLogFiles(base.Server.MapPath("Logs/ErrorLog"));
                    createLogFile.ErrorLog(string.Concat("Metodo FindStudentByRa", exception.Message));
                    dadosAluno = null;
                }
            }
            finally
        ...
        public DocumentsStudent listDocument(string ra, string codcurso, bool requered)
        {
            ...
            try
            {
                ...
                catch (Exception exception1)
                {
                    Exception exception = exception1;
                    CreateLogFiles createLogFile = new CreateLogFiles(base.Server.MapPath("Logs/ErrorLog"));
                    createLogFile.ErrorLog(string.Concat("Metodo ListDocument", exception.Message));
